...
- `getLowStockAlerts` does two queries (products, then `product_stock`) and joins in JS — `product_stock` is a view without a real FK Postgres/PostgREST can use for embedding, so a single embedded query isn't reliable here. `getRecentActivity` also avoids PostgREST relation embedding for the donation→member name lookup (a separate `members` query by id, joined in JS) — not because the FK isn't real, but for consistency: no file in this codebase uses embedded relational selects (`contracts.ts`/`members.ts` always do sequential queries + JS joins), and embedding's to-one/to-many return-shape ambiguity isn't worth introducing as this codebase's first exception.
