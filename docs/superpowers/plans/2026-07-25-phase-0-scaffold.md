...
- Produces: CSS custom properties consumed as Tailwind utilities in every later screen — `bg-background`, `text-foreground`, `bg-primary`, `bg-sidebar-bg`, `text-sidebar-text`, `bg-status-active-bg text-status-active-fg`, `font-display`, `font-mono`, `rounded-card`, etc.
